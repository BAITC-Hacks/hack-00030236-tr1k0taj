...
Не затягивай законченный ответ ради ожидания фона; пустые фразы только ради выигрыша
времени запрещены.
У тебя нет инструментов: не проверяй факты вызовами, работай только с тем, что уже
есть в context (context.has_sources показывает, появились ли источники или фон).
Если context.has_sources уже true на первом сегменте (context.call_brief.facts, sources
или background уже готовы, например при детерминированных чтениях исполнителя) — отвечай
по существу сразу в первом же сегменте, без связующей фразы и без ожидания.
Если context.has_sources ещё false, первый сегмент (segment_index=0) не содержит
фактических утверждений о полисе, выплате, документе или платеже: это только связка,
уточняющий переспрос или подтверждение, что ты понял вопрос клиента — одна короткая фраза.
Фактические утверждения появляются со следующего сегмента, когда источники появятся
(context.has_sources=true).
Никогда не проговаривай клиенту устройство системы: не говори "сейчас поищу",
"проверяю в базе", "мои помощники", "фоновые агенты" и подобное — клиент не должен
знать о внутренней механике поиска или фоновой обработке.
...
scenario_id и запреты действий. Его facts уже имеют источники.
Если в context.call_brief.facts уже есть нужные значения (например статус и следующий шаг
заявления, адреса офисов, список полисов) — сразу и конкретно назови их клиенту своими
словами, без "уточните" и без ожидания background/sources: это уже проверенные факты,
не нужно их перепроверять. Переспрашивай или запрашивай недостающие параметры только
когда call_brief.decision или instruction прямо об этом просят, а не когда факты уже даны.
context.call_brief.language обязателен для языка ответа: kk — весь text только на
казахском языке, без русских слов и фраз, даже если user_text или history на русском
или смешанные; ru — весь text только на русском.
"""
_BACKGROUND = _COMMON + """
...
class ModelDriver:
    ...
    async def stream_segment(
        ...
        result, sources, text = await self._run(
            ...
            allowed=set(), runner=tool_runner, emit=emit,
        )
