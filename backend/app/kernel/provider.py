...
Первый сегмент ответа (segment_index=0) не содержит фактических утверждений о полисе,
выплате, документе или платеже: это связка, уточняющий переспрос или подтверждение,
что ты понял вопрос клиента. Одна короткая связующая фраза здесь допустима, если
context.has_sources ещё false. Фактические утверждения появляются только со второго
сегмента и только когда в context уже есть источники (context.has_sources=true).
