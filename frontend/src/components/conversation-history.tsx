...
export function ConversationHistory() {
  ...
  const { scenarios, systemIntents, scenarioTitle } = useCatalog();
  ...
  const groups = [
    ...
    { key: "scenario", label: t("scenarios"), options: [...scenarios.map(s => s.scenario_id), ...systemIntents.map(s => s.id)].map(value => ({ value, label: `${value} · ${scenarioTitle(value)}` })) },
  ...
  const rows = history.filter(session => {
    ...
    const haystack = redact(`${session.id} ${session.client?.name ?? ""} ${session.claim?.id ?? ""} ${routes.map(id => `${id} ${scenarioTitle(id)}`).join(" ")}`).toLocaleLowerCase();
  ...
  return <><div className="page-heading"><div><div className="eyebrow">CONVERSATION HISTORY</div><h1>{t("historyTitle")}</h1><p>{t("historySubtitle")}</p></div><button className="button secondary" onClick={v.sampleHistory}>{t("sampleHistory")}</button></div>
    ...
    {!rows.length ? <EmptyState icon="search" title={t("noResults")} description={t("changeFilters")}><button className="button secondary" onClick={reset}>{t("reset")}</button></EmptyState> : <div className="table-wrap"><table className="history-table"><thead><tr><th>{t("date")}</th><th>{t("topic")}</th><th>{t("client")}</th><th>{t("language")}</th><th>{t("duration")}</th><th>{t("result")}</th><th><span className="sr-only">{t("details")}</span></th></tr></thead><tbody>{rows.slice((currentPage - 1) * size, currentPage * size).map(session => <tr key={session.id}><td>{new Date(session.startedAt).toLocaleDateString(locale)}<small>{new Date(session.startedAt).toLocaleTimeString(locale, { hour: "2-digit", minute: "2-digit" })}{session.mode === "example" && " · DEMO"}</small></td><td><button onClick={() => { setSelected(session); setTurnId(session.turns.at(-1)?.id ?? null); }}>{session.turns.at(-1)?.scenarios[0] ? scenarioTitle(session.turns.at(-1)!.scenarios[0].scenario_id) : t("noRoute")}</button><small className="mono">{session.id.slice(0, 18)}</small></td><td>{session.client?.name ?? t("notDetermined")}<small>{session.claim?.id ?? ""}</small></td><td>{[...new Set(session.turns.map(turn => turn.language))].join(" / ").toUpperCase()}</td><td className="mono">{duration(session)}</td><td><span className={`badge ${session.outcome === "completed" ? "green" : "amber"}`}>{t(session.outcome ?? "completed")}</span></td><td><button className="icon-button" aria-label={`${t("details")} ${session.id}`} onClick={() => { setSelected(session); setTurnId(session.turns.at(-1)?.id ?? null); }}><Icon name="arrow" size={16} /></button></td></tr>)}</tbody></table></div>}
