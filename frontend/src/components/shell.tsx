...
function Shell({ children }: { children: ReactNode }) {
  ...
  return <div className="app-shell">
    ...
    {guide && <Modal title={t("guide")} onClose={() => setGuide(false)}><div className="guide-steps">{["step1", "step2", "step3", "step4"].map((key, i) => <div key={key}><span>{i + 1}</span><strong>{t(key as "step1")}</strong></div>)}</div><p>{t("guideText")}</p><div className="notice"><Icon name="info" /><p>{t("notConnected")}</p></div></Modal>}
...
export function AppShell({ children }: { children: ReactNode }) { return <VoiceProvider><Shell>{children}</Shell></VoiceProvider>; }
