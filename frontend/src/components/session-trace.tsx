...
import { formatTraceDate, formatTraceMs, traceText, type TraceTextKey } from "@/lib/trace-i18n";
...
function Errors({ errors, locale }: { errors: TraceError[]; locale: Locale }) {
  ...
  return <details className="server-trace-disclosure server-trace-errors" open><summary>{traceText(locale, "errors")} <span>{errors.length}</span></summary>{errors.map(error => <div className="server-trace-error-item" key={`${error.trace_id}:${error.span_id}`}><strong>{error.stage ?? error.span_name}{error.code ? ` · ${error.code}` : ""}</strong>{error.message && <p>{error.message}</p>}<small>{formatTraceDate(error.time, locale)}</small></div>)}</details>;
}
...
function SpanNode({ span, depth, locale, selectTrace }: { span: SpanView; depth: number; locale: Locale; selectTrace: (id: string) => void }) {
  ...
  return <details className={`server-trace-span ${span.status === "error" ? "server-trace-span-error" : ""}`} open={depth === 0}>
    <summary><span className="server-trace-span-name">{span.name}</span><span className="server-trace-ms">{formatTraceMs(span.duration_ms, locale)}</span></summary>
    <div className="server-trace-span-body"><div className="server-trace-span-meta"><span className="badge">{span.kind}</span><span className={`badge ${span.status === "error" ? "amber" : span.status === "ok" ? "green" : ""}`}>{status}</span></div>
      {span.status_message && <p className="server-trace-error-text">{span.status_message}</p>}
...
function SessionTraceContent({ sessionId, live = false, traceId, compact = false }: Props) {
  ...
  const link = `/traces?session=${encodeURIComponent(sessionId)}${selected ? `&trace=${encodeURIComponent(selected)}` : ""}`;
  ...
  return <section className={`server-trace ${compact ? "server-trace-compact" : ""}`} aria-label={tr("title")}>
    ...
    {summary && <>
      <div className="server-trace-metrics">{([ ["turns", summary.turns], ["traces", summary.traces], ["calls", summary.llm_calls], ["errors", summary.errors] ] as const).map(([label, value]) => <div key={label}><span>{tr(label)}</span><strong className={label === "errors" && value ? "server-trace-error-text" : ""}>{value.toLocaleString(locale)}</strong></div>)}</div>
      ...
      {turn && <section className="server-trace-section"><h4>{tr("stages")} · {turn.turn_id ?? "—"}</h4><dl className="server-trace-data"><div><dt>{tr("source")}</dt><dd>{turn.input_source ?? "—"}</dd></div><div><dt>{tr("decision")}</dt><dd>{turn.router.decision ?? "—"}</dd></div><div><dt>{tr("confidence")}</dt><dd>{turn.router.confidence === null ? "—" : turn.router.confidence.toLocaleString(locale, { maximumFractionDigits: 3 })}</dd></div></dl>
        {Object.keys(turn.stages).length ? <dl className="server-trace-data server-trace-stages">{Object.entries(turn.stages).map(([name, duration]) => <div key={name}><dt>{name}</dt><dd>{formatTraceMs(duration, locale)}</dd></div>)}</dl> : <p className="server-trace-note">{tr("noStages")}</p>}
        <ModelTokens models={turn.tokens_by_model} locale={locale} /><Errors errors={turn.errors} locale={locale} />
      </section>}
      <section className="server-trace-section"><h4>{tr("tree")}</h4>
        {!!session.data!.traces.length && <label className="server-trace-select">{tr("trace")}<select value={selected ?? ""} onChange={event => selectTrace(event.target.value)}>{selected && !session.data!.traces.some(item => item.trace_id === selected) && <option value={selected}>{tr("linked")} · {selected.slice(0, 12)}</option>}{session.data!.traces.map(item => <option key={item.trace_id} value={item.trace_id}>{item.turn_id !== null ? `${tr("turn")} ${item.turn_id} · ` : ""}{item.root_name} · {item.trace_id.slice(0, 8)}</option>)}</select></label>}
