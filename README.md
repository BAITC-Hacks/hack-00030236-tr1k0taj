...
| `just eval` (`just eval-local` без Docker) | оценка роутера на 104 dev-репликах через `evaluate.py` кита; нужен `OPENAI_API_KEY` |
...
Прочерки — `just eval` ещё не запускался с реальным `OPENAI_API_KEY`. Запуск: `OPENAI_API_KEY=... just eval` (или `just eval-local` без Docker), таблица и `router_eval_result.json` — в `backend/evals/`.
...
Дата набора — **2026-10-01**. Разметка `expected` из dev-набора предназначена только для evaluator (`backend/evals/router.py`) и не передаётся роутеру. `just eval` прогоняет реальный роутер по всем 104 репликам и вызывает исходный `evaluate.py`; без `OPENAI_API_KEY` — понятная ошибка, а не мок.
