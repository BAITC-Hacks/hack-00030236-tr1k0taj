...
## Главная особенность: голос не ждёт агентов

Обычный пайплайн «LLM-агенты → собрать ответ → озвучить» заставляет клиента молчать, пока
все модели не допишут. У нас **голосовой агент начинает отвечать сразу**, а сценарные и
фоновые агенты работают параллельно и подмешивают свои результаты в разговор **по ходу**,
через общую доску — **blackboard**.

- **Blackboard** — общее состояние звонка (`context` + журнал `kernel`): реплики, выбранный
  сценарий, слоты, факты с `source_id`, результаты агентов. Каждая запись версионирована
  (`generation`, `input_revision`), устаревшие результаты не попадают в ответ.
- **Голосовой агент (main)** генерирует ответ короткими **сегментами** и стримит их в TTS.
  Перед каждым следующим сегментом он заново читает доску и использует всё, что успело
  появиться, — уже опубликованный текст не переписывается.
- **Фоновые агенты** (поиск по базе знаний, следующий шаг, чтения данных клиента) стартуют
  вместе с ходом, работают параллельно и публикуют факты на доску. Их никто не ждёт до
  конца: успели — попадут в следующий сегмент, не успели — останутся для следующей реплики.
- **Параллельно с роутером** уже идёт поиск по базе знаний, а первая фраза-подтверждение
  звучит сразу после выбора сценария — клиент не слышит тишины.

```mermaid
sequenceDiagram
    participant C as Клиент
    participant R as Роутер
    participant V as Голосовой агент
    participant B as Blackboard
    participant A as Фоновые агенты
    C->>R: реплика
    R-->>B: сценарий, слоты
    par сразу
        V->>C: «Секунду, проверяю» + сегмент 1 (стрим TTS)
    and параллельно
        A->>B: факты из базы и данных клиента (с source_id)
    end
    V->>B: читает свежие факты
    V->>C: сегмент 2 уже с фактами агентов
```

Итог: задержка до первого звука определяется роутером, а не самым медленным агентом,
при этом ответ остаётся основанным на данных.
...
озвучивает уже готовые предложения, не дожидаясь всего ответа. В критическом пути до
первого звука — один LLM-вызов роутера; ответ и фоновые агенты идут параллельно (ADR 0011).
